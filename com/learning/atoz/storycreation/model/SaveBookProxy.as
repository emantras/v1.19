package com.learning.atoz.storycreation.model
{
	...
	*/
	public class SaveBookProxy extends Proxy implements IProxy
	{		
		public static const NAME:String = "SaveBookProxy";
		private var request:URLRequest;
		private var myLoader:URLLoader;
		private var saveurl:String='http://emantras.raz-kids.com/main/CreativeWritingSaveLoad/action/saveBook';						
		//-----------------------------------------------------
		public function SaveBookProxy() 
        {
            super ( NAME,this );
		...
		//-----------------------------------------------------
		public function saveBook(bookxml:String,bookid:Number):void
		{			
			facade.sendNotification(ApplicationConstants.SHOW_WAIT_POPUP,"Saving Book.....");			
			try
			{
				var sbxml:XML=new XML(bookxml);
				trace("savebook1 page cnt:"+sbxml..page.length());
				if(sbxml..page.length()>0)
				{
					var bgcnt:int=sbxml..background.length();
					var objcnt:int=sbxml..data.length();
					trace("savebook2 bgcnt="+bgcnt+",objcnt="+objcnt);
					
					if(bookid>0 && (bgcnt>0 || objcnt>0))
					{
						request = new URLRequest(saveurl+"/id/"+bookid);
										
						//trace("saveBook=>"+saveurl+"/id/"+bookid);
		...
		{	
			...
			trace("SaveBookProxy=>onLoadComplete=>"+rawData);
			facade.sendNotification(ApplicationConstants.CLOSE_WAIT_POPUP);
		}
		//-----------------------------------------------------
